    ...
    return (
      <Card className="w-full border-0 bg-gradient-to-br from-red-50 to-orange-50 shadow-lg">
        ...
        <CardContent className="pt-6">
          ...
          <Button onClick={fetchInventoryData} variant="outline" className="border-red-300 text-red-700 hover:bg-red-50">
        ...
        <CardTitle className="flex items-center gap-2 text-lg font-semibold text-orange-800">
          ...
          <span className="ml-auto flex items-center gap-2">
            <span className="text-sm font-medium text-orange-600">
              {loading ? "..." : inventoryData ? `${inventoryData.lowStockCount} alerts` : "0 alerts"}
            </span>
            {expanded ? (
        ...
        {inventoryData && inventoryData.items.length > 0 ? (
          <div className="space-y-4">
            <div className="flex justify-between items-center">
              ...
              <Button onClick={fetchInventoryData} size="sm" variant="outline" className="border-orange-300 text-orange-700 hover:bg-orange-50">
                <RefreshCw className="h-4 w-4 mr-2" />
                Refresh
              </Button>
            </div>
              ...
              <TableBody>
                {inventoryData.items.map((item) => (
                  <TableRow key={item.id} className="border-orange-100 hover:bg-orange-50/50">
                    ...
                    <TableCell className={getStockStatusColor(item.currentStock, item.lowStockThreshold)}>
                      {item.currentStock}
                    </TableCell>
          ...
          <div className="text-center py-8">
            ...
            <p className="text-orange-700 font-medium mb-2">All Items Well Stocked!</p>
            <p className="text-orange-600 text-sm">No low stock alerts at this time</p>
          </div>
        )}
      </CardContent>
      )}
