...
export const PrintButton: React.FC = () => {
  ...
  const createTestLabel = (): string => {
    ...
    ctx.fillText(
      "Printer: " + (selectedPrinter || connectionInfo.defaultPrinter?.name || "Default"),
      width / 2,
      260
    )
  ...
  return (
    <div className="space-y-4 rounded-lg border p-4">
      ...
      <div className="text-sm">
        ...
        {connectionInfo.defaultPrinter?.name || "None"}
      </div>
        ...
        <div className="flex justify-center">
          <div className="rounded-lg border-2 border-purple-300 bg-white p-2">
            <img
              ...
              style={{
                width: labelHeight === "40mm" ? "124px" : "248px",
                height: labelHeight === "40mm" ? "70px" : "140px",
              }}
            />
          </div>
        </div>
        <div className="text-center text-xs text-gray-500">56mm x {labelHeight} test label</div>
      </div>

      {printStatus && <div className="rounded bg-gray-100 p-2 text-sm">{printStatus}</div>}

      <button
        onClick={handlePrint}
        disabled={!connectionInfo.isConnected || connectionInfo.loading || !selectedPrinter}
        className="w-full rounded bg-purple-600 px-4 py-2 text-white hover:bg-purple-700 disabled:cursor-not-allowed disabled:bg-gray-400"
      >
