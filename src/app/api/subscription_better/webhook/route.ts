    ...
    switch (event.type) {
      case "customer.subscription.created": {
          ...
          if (isFirstSubscription) {
            ...
            await sendMail({
              ...
              body: planChangeEmail({
                ...
                changeType:
                  (sub.metadata?.plan_change_type as "upgrade" | "downgrade" | "billing_change") ||
                  "upgrade",
                effectiveDate: sub.metadata?.plan_change_effective
                  ? new Date(Number(sub.metadata.plan_change_effective) * 1000).toLocaleDateString()
                  : "",
                amount: sub.items.data[0]?.price?.unit_amount
                  ? sub.items.data[0].price.unit_amount / 100
                  : 0,
        ...
        if (subscriptionPlanId) {
          try {
            ...
            if (planResult.rows.length > 0) {
              ...
              if (plan.devices_included && plan.devices_included > 0) {
                ...
                try {
                  await sendMail({
                    ...
                    body: `
                      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
                        ...
                        <p>You can manage this in the admin dashboard at: <a href="https://www.instalabel.co/bossdashboard/devices">Device Management</a></p>
                  ...
                  console.log(
                    `[ADMIN NOTIFICATION] Device notification sent to bib3krzl@gmail.com for ${isUpgrade ? "upgrade" : "signup"} - user ${user_id}`
                  )
                } catch (adminEmailError) {
                  console.error(
                    `[ADMIN NOTIFICATION] Failed to send admin email for user ${user_id}:`,
                    adminEmailError
                  )
                }
              }
            ...
            console.error(
              `[ADMIN NOTIFICATION] Error checking device requirements for user ${user_id}:`,
              deviceError
            )
          }
        }
        ...
        const plan_name =
          sub.metadata?.plan_name || getPlanNameFromPriceId(price_id) || price_id || ""
        const plan_interval =
          sub.metadata?.plan_interval || sub.items.data[0]?.price?.recurring?.interval || null
        ...
        try {
          ...
          if (plan_id) {
            ...
            const planRes = await client.query("SELECT include_device FROM plans WHERE id = $1", [
              plan_id,
            ])
            if (planRes.rows[0]?.include_device) {
              // Check if user already has a device for this plan
              const existingDevice = await client.query(
                "SELECT id FROM devices WHERE user_id = $1 AND plan_id = $2",
                [user_id, plan_id]
              )
              if (existingDevice.rows.length === 0) {
                ...
                console.log(
                  `[WEBHOOK] Device assignment created for user_id ${user_id}, plan_id ${plan_id}`
                )
              } else {
                console.log(
                  `[WEBHOOK] Device already assigned for user_id ${user_id}, plan_id ${plan_id}`
                )
              }
            }
          }
      ...
      case "customer.subscription.updated": {
        ...
        try {
          ...
          if (
            ...
            Math.abs(new Date(dbSub.pending_plan_change_effective).getTime() - Date.now()) <
            ...
            await stripe.subscriptions.update(sub.id, {
              metadata: {
                ...
                plan_change_effective: Math.floor(
                  new Date(dbSub.pending_plan_change_effective).getTime() / 1000
                ).toString(),
              },
            })
            ...
            if (userEmail) {
              await sendMail({
                ...
                body: planChangeEmail({
                  ...
                  effectiveDate: dbSub.pending_plan_change_effective
                    ? new Date(dbSub.pending_plan_change_effective).toLocaleDateString()
                    : "",
                  amount: sub.items.data[0]?.price?.unit_amount
                    ? sub.items.data[0].price.unit_amount / 100
                    : 0,
            ...
            if (dbSub.pending_plan_change_type === "upgrade" && dbSub.pending_plan_change) {
              try {
                const newPlanResult = await pool.query("SELECT * FROM plans WHERE id = $1", [
                  dbSub.pending_plan_change,
                ])
                ...
                if (newPlanResult.rows.length > 0) {
                  ...
                  if (newPlan.devices_included && newPlan.devices_included > 0) {
                    ...
                    try {
                      ...
                      console.log(
                        `[ADMIN NOTIFICATION] Upgrade device notification sent to bib3krzl@gmail.com for user ${user_id}`
                      )
                    } catch (adminEmailError) {
                      console.error(
                        `[ADMIN NOTIFICATION] Failed to send upgrade admin email for user ${user_id}:`,
                        adminEmailError
                      )
                    }
                  }
                ...
                console.error(
                  `[ADMIN NOTIFICATION] Error checking upgrade device requirements for user ${user_id}:`,
                  deviceError
                )
              }
            }
            ...
            if (dbSub.pending_plan_change_type === "downgrade") {
              try {
                ...
                const oldPlanResult = await pool.query("SELECT * FROM plans WHERE id = $1", [
                  dbSub.plan_id,
                ])
                ...
                if (oldPlanResult.rows.length > 0 && newPlanResult.rows.length > 0) {
                  ...
                  if (
                    oldPlan.devices_included &&
                    oldPlan.devices_included > 0 &&
                    ...
                    console.log(
                      `[DEVICE RETURN] Marked devices for return for user ${user_id} due to plan downgrade`
                    )
                    ...
                    if (userEmail) {
                      try {
                        ...
                        console.log(
                          `[DEVICE RETURN] Downgrade notification email sent to ${userEmail}`
                        )
                      } catch (emailError) {
                        console.error(
                          `[DEVICE RETURN] Failed to send downgrade email to ${userEmail}:`,
                          emailError
                        )
                      }
                    }
                ...
                console.error(
                  `[DEVICE RETURN] Error handling device return for downgrade user ${user_id}:`,
                  deviceError
                )
              }
            }
        ...
        if (userEmail && (sub as any)["current_period_end"]) {
          const daysLeft = Math.ceil(
            ((sub as any)["current_period_end"] * 1000 - Date.now()) / (1000 * 60 * 60 * 24)
          )
          if (daysLeft === 7) {
            await sendMail({
              to: userEmail,
        ...
        if (userEmail && sub.trial_end) {
          const trialDaysLeft = Math.ceil(
            (sub.trial_end * 1000 - Date.now()) / (1000 * 60 * 60 * 24)
          )
        ...
        if (
          ...
          if (now - start < 1000 * 60 * 60 * 24) {
            // within 24 hours of renewal
            await sendMail({
              to: userEmail,
      ...
      case "checkout.session.completed": {
        ...
        if (session.mode === "payment" && session.metadata?.order_id) {
          ...
          try {
            ...
            if (order && order.label_product_id) {
              const productRes = await client.query("SELECT * FROM label_products WHERE id = $1", [
                order.label_product_id,
              ])
              product = productRes.rows[0] || {}
            }
            if (order && order.email) {
              console.log("[WEBHOOK] Sending label order confirmation to user:", {
                to: order.email,
                orderId: order.id,
              })
              await sendMail({
              ...
              console.log("[WEBHOOK] Sent label order confirmation to user:", {
                to: order.email,
                orderId: order.id,
              })
              console.log("[WEBHOOK] Sending label order confirmation to admin:", {
              ...
              console.log("[WEBHOOK] Sent label order confirmation to admin:", {
                to: "admin@instalabel.co",
                orderId: order.id,
              })
            }
        ...
        const plan_interval =
          sub.metadata?.plan_interval || sub.items.data[0]?.price?.recurring?.interval || null
        console.log(
          "[WEBHOOK] Upserting with plan_id:",
          plan_id,
          "price_id:",
          price_id,
          "plan_name:",
        ...
        try {
          ...
          console.log(
            "[WEBHOOK] Upserted subscription for user_id (checkout.session.completed):",
            user_id
          )
          // --- DEVICE ASSIGNMENT LOGIC ---
          if (plan_id) {
