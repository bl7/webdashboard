...
export async function GET(req: NextRequest) {
  try {
    ...
    const monthlyActive = subs.filter(
      (s: any) => s.status === "active" && s.billing_interval === "month"
    ).length
    const annualActive = subs.filter(
      (s: any) => s.status === "active" && s.billing_interval === "year"
    ).length
    const canceled = subs.filter((s: any) => s.status === "canceled").length
    const mrr =
      subs
        .filter((s: any) => s.status === "active" && s.billing_interval === "month")
        .reduce((sum: any, s: any) => sum + (s.amount || 0) / 100, 0) +
      subs
        .filter((s: any) => s.status === "active" && s.billing_interval === "year")
        .reduce((sum: any, s: any) => sum + (s.amount || 0) / 100 / 12, 0)
    const arr =
      subs
        .filter((s: any) => s.status === "active" && s.billing_interval === "year")
        .reduce((sum: any, s: any) => sum + (s.amount || 0) / 100, 0) +
    ...
    const trialConversion =
      trialing + active + canceled > 0 ? (active + canceled) / (trialing + active + canceled) : 0
    // Plan distribution
    const planMap: Record<string, number> = {}
    subs.forEach((s: any) => {
      if (s.plan_name) planMap[s.plan_name] = (planMap[s.plan_name] || 0) + 1
    })
    ...
    const topCustomers = [...subs]
      .sort((a: any, b: any) => (b.amount || 0) / 100 - (a.amount || 0) / 100)
      .slice(0, 10)
